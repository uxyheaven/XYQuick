...
- (BOOL)send
{
    if ( self.source )
    {
        ...
        if ( sourceView )
        {
            ...
            if ( nameSpace )
            {
                selector = [NSString stringWithFormat:@"%@_%ld", nameSpace, (long)tag];
            }
...
- (XYUISignal *)sendUISignal:(NSString *)name withObject:(NSObject *)object from:(id)source
{
    ...
    return [self sendUISignal:name withObject:object from:source to:self];
}
...
- (XYUISignal *)sendUISignal:(NSString *)name withObject:(NSObject *)object from:(id)source to:(id)target
{
    ...
    if ( signal )
    {
        signal.source = source ?: self;
        signal.target = target ?: self;
        signal.name   = name;
